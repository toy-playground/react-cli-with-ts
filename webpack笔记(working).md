...
我理解的webpack实际上就是一个多模块融合成庞大文件的工具，文档里叫它**静态模块打包工具**
它所提供的是模块化的解决方案。

### 配置项

webpack作为一个工具，必然会留下配置项，配置方式:

1. 通过在项目的根目录下创建`webpack.config.js`文件来进行配置。
2. 使用其他配置文件，通过`scripts`中添加 `web pack --config yourfile`来进行配置。

这是webpack官网给出的一个示例，我们先贴出来，然后一点一点看，当然可以先跳过去- - 毕竟第一次看都看不懂
...
entry: {
  entry1: {
    entry1: './app/entry1',
    entry2: {
      dependOn: 'entry1',// 必须在当前入口加载前进行加载
      fileName: '', // 输出的文件名称
      import: './app/entry2', // 启动时需要加载的模块
      library: ''// 不太会用 官方解释是 为当前 entry 构建一个 library。
      ...
    }
  }
}
